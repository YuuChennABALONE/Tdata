/* "AMCAX_API BoolBRepSection(const std::shared_ptr<Geom3Surface>& s1, const std::shared_ptr<Geom3Surface>& s2, bool perfomNow = true)\t @param perfomNow If true, the operation is performed immediately; if false, the Build() function has to be called manually\n\t @param s2 The tool surface\n\t @param s1 The argument surface\n\t @brief Special case: compute the section of two surfaces\n,\n",
      */
 #include<catch.hpp>
