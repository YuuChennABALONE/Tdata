/* "AMCAX_API BoolBRepSection(const TopoShape& s1, const std::shared_ptr<Geom3Surface>& surface, bool perfomNow = true)\t @param perfomNow If true, the operation is performed immediately; if false, the Build() function has to be called manually\n\t @param surface The tool surface\n\t @param s1 The argument shape\n\t @brief Special case: compute the section of a shape and a surface\n,\n",
      */
 #include<catch.hpp>
