/* "AMCAX_API BoolBRepSection(const std::shared_ptr<Geom3Surface>& surface, const TopoShape& s2, bool perfomNow = true)\t @param perfomNow If true, the operation is performed immediately; if false, the Build() function has to be called manually\n\t @param s2 The tool shape\n\t @param surface The argument surface\n\t @brief Special case: compute the section of a surface and a shape\n,\n",
      */
 #include<catch.hpp>
